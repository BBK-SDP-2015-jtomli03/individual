...
public class MulInstructionTest {
    ...
    private MulInstruction ins;
    private MulInstruction ins2;
    private Machine m;

    @Before
    public void setUp() throws Exception {
        ins = new MulInstruction("f1", 1, 2, 3);
        m = new Machine();
    ...
    public void testExecuteThrowsExceptionIfResultRegisterParameterNotInRange(){
        ins2 = new MulInstruction("f1", 33, 2, 3); //register 33 doesn't exist
        ins2.execute(m);
    }

    @Test(expected = ArrayIndexOutOfBoundsException.class)
    public void testExecuteThrowsExceptionIfOp1RegisterParameterNotInRange(){
        ins2 = new MulInstruction("f1", 2, 33, 3); //register 33 doesn't exist
        ins2.execute(m);
    }

    @Test(expected = ArrayIndexOutOfBoundsException.class)
    public void testExecuteThrowsExceptionIfOp2ResultRegisterParameterNotInRange(){
        ins2 = new MulInstruction("f1", 2, 2, 33); //register 33 doesn't exist
        ins2.execute(m);
    }
